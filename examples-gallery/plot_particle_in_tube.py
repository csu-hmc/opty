...
A particle of mass :math:`m` is moving from a starting point to an ending point,
subject to a viscous friction force and to a uniform gravitational field.
The particle must not leave a tube defined by a curve (centerline) in space and a
radius. At one point during the motion, it must pass through a narrow gate,
...
eval_coords = sm.lambdify((state_symbols, fx, fy, fz,
...
def plot_3d_circle(ax, center, radius, normal, num_points=100):
    ...
    circle_3d = (center[:, np.newaxis] + v1[:, np.newaxis] * circle_points[0, :]
...
def animate(i):
    ...
    pfeil.remove()
    pfeil = ax.quiver([coords[0, 0]], [coords[1, 0]], [coords[2, 0]],
