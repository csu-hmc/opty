...
delta, F = me.dynamicsymbols('delta, F', real=True)
...
fr, frstar = kane.kanes_equations([car], [propulsion])

eom = (fr + frstar).col_join(
    sm.Matrix(nonholonomic)).col_join(sm.Matrix(kinematical))
sm.pprint(eom)
...
state_symbols = (x, y, theta, vx, vy, omega)
specified_symbols = (delta, F)
...
obj_func = sm.Integral(F**2/10000 + delta**2, t)
sm.pprint(obj_func)
